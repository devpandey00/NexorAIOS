...
async function processExistingLeadBatch() {
  ...
  const leads = await prisma.lead.findMany({
    where: { status: { in: [LeadStatus.NEW, LeadStatus.RESEARCHED] }, whatsapp: { not: null } },
  ...
  for (const lead of leads) {
    try {
      ...
      if (lead.website) {
        ...
        research = result;
        if (result.success) {
          intelligence = assessLead({
            website: result.website,
            technology: result.technology,
            social: Object.fromEntries(Object.entries(result.social ?? {})),
            seo: Object.fromEntries(Object.entries(result.seo ?? {})),
          });
          researched++;
        }
      }
      ...
      await prisma.lead.update({
        where: { id: lead.id },
        data: { auditScore: score, status: score >= 70 ? LeadStatus.QUALIFIED : LeadStatus.RESEARCHED, notes },
      });

      if (!whatsappVerified) continue;

      const existing = await prisma.outreach.findFirst({
        where: { leadId: lead.id, channel: OutreachChannel.WHATSAPP, status: { notIn: [OutreachStatus.CANCELLED, OutreachStatus.FAILED] } },
      });
      ...
      const outreach = await prisma.outreach.create({
        data: {
          ...
          status: autoSendEnabled ? OutreachStatus.APPROVED : OutreachStatus.APPROVAL_REQUIRED,
          message,
        },
      });
      drafted++;
...
export async function runAutopilot() {
  ...
  for (const item of [...opportunities.companies, ...opportunities.influencers]) {
    ...
    try {
      ...
      // Keep discovery usable when a target blocks research.
    }
    ...
    if (!existingDraft) {
      await prisma.outreach.create({ data: { leadId: lead.id, channel: lead.email ? OutreachChannel.EMAIL : OutreachChannel.WHATSAPP, status: OutreachStatus.DRAFT, message } });
      opportunityDrafts++;
    }
  }
