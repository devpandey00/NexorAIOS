...
export async function sendNexorReportEmail(periodHours = 24) {
  const apiKey = process.env.RESEND_API_KEY?.trim(); const from = process.env.REPORT_FROM_EMAIL?.trim() || process.env.OUTREACH_FROM_EMAIL?.trim(); const to = process.env.REPORT_EMAIL_TO?.trim();
  if (!apiKey) throw new Error('RESEND_API_KEY is not configured'); if (!from) throw new Error('REPORT_FROM_EMAIL or OUTREACH_FROM_EMAIL is not configured'); if (!to) throw new Error('REPORT_EMAIL_TO is not configured');
