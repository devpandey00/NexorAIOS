...
function parseLeadMetadata(notes: string | null) {
  try { const parsed = notes ? JSON.parse(notes) : {}; return parsed?.metadata ?? parsed ?? {}; } catch { return {}; }
}

function leadIsSendable(lead: { businessName: string; whatsapp: string | null; notes: string | null; country?: string | null }) {
...
export function getWhatsAppProviderStatus() {
  const openwa=getOpenWAStatus(); const config=whatsappConfig();
  return {configured:openwa.configured||config.configured,mode:openwa.configured?'openwa':(config.templateName?'template':'template_required'),openwaConfigured:openwa.configured,templateConfigured:Boolean(config.templateName),templateLanguage:config.templateLanguage,optInRequired:true};
}

async function sendWhatsApp(to:string, message:string, templateParameter:string) {
  ...
  if(!token||!phoneNumberId) throw new Error('WhatsApp provider is not configured: add Meta Cloud API credentials in Vercel Production.');
  if(!templateName) throw new Error('WhatsApp first-contact sending is paused: an approved Meta message template is required. Set WHATSAPP_TEMPLATE_NAME and WHATSAPP_TEMPLATE_LANGUAGE after Meta approves the template.');
  const recipient=to.replace(/\D/g,'');
  if(!recipient||recipient.length<8) throw new Error('Lead WhatsApp number is invalid after normalization.');
  const parameterText=templateParameter.replace(/[\r\n\t]+/g,' ').replace(/\s{2,}/g,' ').trim();
  if(!parameterText) throw new Error('WhatsApp template parameter is empty.');
  const payload={messaging_product:'whatsapp',recipient_type:'individual',to:recipient,type:'template',template:{name:templateName,language:{code:templateLanguage},components:[{type:'body',parameters:[{type:'text',text:parameterText}]}]}};
  const response=await fetch(`https://graph.facebook.com/${version}/${phoneNumberId}/messages`,{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify(payload),cache:'no-store'});
  const data=await response.json().catch(()=>({}));
  if(!response.ok){const providerMessage=data?.error?.message??`WhatsApp send failed (${response.status})`;const providerCode=Number(data?.error?.code??0);const code=providerCode?` [Meta ${providerCode}]`:'';if(providerCode===190) throw new Error('Meta WhatsApp authentication failed (Meta 190): the Production WHATSAPP_ACCESS_TOKEN is invalid, expired, or no longer authorized.');throw new Error(`${providerMessage}${code}`);}
  return data?.messages?.[0]?.id as string|undefined;
}

export async function sendEmail(to:string,message:string){
  const apiKey=process.env.RESEND_API_KEY; const from=process.env.OUTREACH_FROM_EMAIL||process.env.REPORT_FROM_EMAIL;
  if(!apiKey||!from) throw new Error('Email credentials are not configured');
  const lines=message.split('\n'); const subject=lines[0]?.startsWith('Subject:')?lines[0].replace(/^Subject:\s*/i,'').trim():'A quick observation about your business'; const text=lines[0]?.startsWith('Subject:')?lines.slice(2).join('\n'):message;
  ...
  const data=await response.json().catch(()=>({})); if(!response.ok) throw new Error(data?.message??`Email send failed (${response.status})`); return data?.id as string|undefined;
}

export async function sendReportEmail(subject:string,text:string){const to=process.env.REPORT_EMAIL_TO?.trim();if(!to) throw new Error('REPORT_EMAIL_TO is not configured');return sendEmail(to,`Subject: ${subject}\n\n${text}`);}
...
export async function sendApprovedOutreach(id:string){
  ...
  if(outreach.channel===OutreachChannel.WHATSAPP){const validation=leadIsSendable(outreach.lead);if(!validation.ok) throw new Error(validation.reason);}
  if(MANUAL_SOCIAL_CHANNELS.has(outreach.channel)){
    const profile=outreach.lead.socialProfiles.find((item)=>item.platform===outreach.channel); if(!profile?.url) throw new Error(`No saved ${outreach.channel} profile URL for manual send`);
    ...
    if(claimed.count!==1){const current=await prisma.outreach.findUnique({where:{id}});if(current?.status===MANUAL_PENDING||current?.status===OutreachStatus.SENT)return {outreach:current,recipient:profile.url,manual:true,alreadySent:current.status===OutreachStatus.SENT};throw new Error('Outreach was claimed by another sender');}
  ...
  try{
    ...
    const result=await prisma.$transaction(async(tx)=>{
      const sent=await tx.outreach.updateMany({where:{id,status:OutreachStatus.SCHEDULED},data:{status:OutreachStatus.SENT,sentAt:now,providerMessageId,error:null}});if(sent.count!==1) throw new Error('Outreach state changed while provider was sending');
  ...
  }catch(error){const message=error instanceof Error?error.message:String(error);await prisma.outreach.updateMany({where:{id,status:OutreachStatus.SCHEDULED},data:{status:OutreachStatus.FAILED,error:message}}).catch(()=>undefined);throw new Error(message);}
}

export async function confirmManualOutreachSent(id:string){
  const prisma=getPrisma();const existing=await prisma.outreach.findUnique({where:{id},include:{lead:true}});if(!existing) throw new Error('Outreach not found');if(existing.status===OutreachStatus.SENT)return {outreach:existing,alreadySent:true};if(existing.status!==MANUAL_PENDING)throw new Error('Outreach is not awaiting manual confirmation');const now=new Date();
  const result=await prisma.$transaction(async(tx)=>{const confirmed=await tx.outreach.updateMany({where:{id,status:MANUAL_PENDING},data:{status:OutreachStatus.SENT,sentAt:now,error:null}});if(confirmed.count!==1)throw new Error('Manual send was already confirmed by another request');const channel=existing.channel as unknown as ConversationChannel;const conversation=await tx.conversation.upsert({where:{leadId_channel:{leadId:existing.leadId,channel}},create:{leadId:existing.leadId,channel,status:'OPEN',lastMessageAt:now},update:{lastMessageAt:now,status:'OPEN'}});await tx.message.create({data:{conversationId:conversation.id,direction:MessageDirection.OUTBOUND,content:existing.message}});await tx.lead.update({where:{id:existing.leadId},data:{status:LeadStatus.CONTACTED}});const existingFollowUp=await tx.followUp.findFirst({where:{leadId:existing.leadId,status:{in:[FollowUpStatus.PENDING,FollowUpStatus.SCHEDULED]}}});if(!existingFollowUp)await tx.followUp.create({data:{leadId:existing.leadId,status:FollowUpStatus.PENDING,scheduledAt:new Date(now.getTime()+3*86400000),notes:`Follow up after manual outreach ${existing.id}`}});return tx.outreach.findUnique({where:{id}});});return {outreach:result,alreadySent:false};
}
