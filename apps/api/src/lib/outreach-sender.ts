...
export async function sendApprovedOutreach(id: string) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!existingFollowUp) {
      await tx.followUp.create({ data: { leadId: outreach.leadId, status: FollowUpStatus.PENDING, scheduledAt: new Date(now.getTime() + 3 * 86400000), notes: `Follow up after outreach ${outreach.id}` } });
    }
