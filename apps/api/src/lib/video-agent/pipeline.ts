...
export async function createVideoSession(input: { filename: string; contentType: string; size: number }): Promise<Json> {
  ...
  try {
    ...
    const uploadUrl = String(slot.uploadUrl || '').trim();
    const sessionId = String(importSession.sessionId || '').trim();
...
export async function finalizeAndBuild(input: { projectId: string; receipt: string; durationInSeconds: number; width?: number; height?: number; fps?: number; hasAudioTrack?: boolean; count: number; instruction?: string }): Promise<Json> {
  ...
  try {
    ...
    const durationFrames = Math.max(1, Math.round(input.durationInSeconds * (input.fps || 30)));
    await client.callTool('edit_item', { adds: [{ type: 'video', assetId, track: 'V1', fromFrame: 0, durationInFrames: durationFrames }] });
    ...
    const audio = await client.callTool('list_audio', {});
    const audioRows = Array.isArray(audio) ? audio.map(obj) : [];
    ...
    if (preferred?.name) {
      ...
      const timeline = obj(await client.callTool('read_timeline', {}));
      const items = Array.isArray(timeline.items) ? timeline.items.map(obj) : [];
    ...
    for (const short of shorts) {
      const timelineId = String(short.timelineId || '').trim();
...
export async function renderStatus(input: { projectId: string; renderId: string }): Promise<Json> {
  const client = new OpenChatCutMcpClient();
  try { await client.callTool('target_project', { projectId: input.projectId }); return { ...obj(await client.callTool('track_export', { action: 'status', renderId: input.renderId })), projectId: input.projectId, renderId: input.renderId }; }
  finally { await client.close(); }
}
