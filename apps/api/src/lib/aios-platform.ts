      ...
      initializing = (async () => {
        await prisma.$executeRawUnsafe(`
          CREATE TABLE IF NOT EXISTS public.aios_companies (
            ...
            website varchar(1000), industry varchar(150), location varchar(255),
            owner_name varchar(255), notes text, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
            UNIQUE(name, website)
          );
          CREATE TABLE IF NOT EXISTS public.aios_contacts (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), company_id uuid REFERENCES public.aios_companies(id) ON DELETE CASCADE,
            lead_id uuid REFERENCES public.leads(id) ON DELETE SET NULL, name varchar(255), email varchar(255), phone varchar(50),
            linkedin varchar(1000), instagram varchar(1000), created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
          );
          CREATE TABLE IF NOT EXISTS public.aios_approvals (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), action varchar(100) NOT NULL, target_type varchar(100) NOT NULL, target_id varchar(255),
            payload jsonb NOT NULL DEFAULT '{}'::jsonb, reason text, status varchar(40) NOT NULL DEFAULT 'PENDING',
            created_by uuid, approved_by uuid, created_at timestamptz NOT NULL DEFAULT now(), approved_at timestamptz, executed_at timestamptz,
          ...
          CREATE TABLE IF NOT EXISTS public.aios_automations (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name varchar(255) NOT NULL, description text, enabled boolean NOT NULL DEFAULT true,
            trigger_type varchar(100) NOT NULL, conditions jsonb NOT NULL DEFAULT '[]'::jsonb, actions jsonb NOT NULL DEFAULT '[]'::jsonb,
            created_by uuid, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
          );
          CREATE TABLE IF NOT EXISTS public.aios_automation_runs (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), automation_id uuid NOT NULL REFERENCES public.aios_automations(id) ON DELETE CASCADE,
            status varchar(40) NOT NULL DEFAULT 'QUEUED', trigger_payload jsonb, result jsonb, error text, attempts int NOT NULL DEFAULT 0,
          ...
          CREATE TABLE IF NOT EXISTS public.aios_invoices (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), invoice_number varchar(80) NOT NULL UNIQUE, client_name varchar(255) NOT NULL,
            client_email varchar(255), opportunity_id uuid REFERENCES public.opportunities(id) ON DELETE SET NULL, subtotal numeric(12,2) NOT NULL DEFAULT 0,
            discount numeric(12,2) NOT NULL DEFAULT 0, tax numeric(12,2) NOT NULL DEFAULT 0, total numeric(12,2) NOT NULL DEFAULT 0,
            currency varchar(10) NOT NULL DEFAULT 'INR', status varchar(40) NOT NULL DEFAULT 'DRAFT', issue_date timestamptz NOT NULL DEFAULT now(),
            due_date timestamptz, notes text, created_by uuid, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
          );
          CREATE TABLE IF NOT EXISTS public.aios_invoice_items (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), invoice_id uuid NOT NULL REFERENCES public.aios_invoices(id) ON DELETE CASCADE,
            description varchar(500) NOT NULL, quantity numeric(12,2) NOT NULL DEFAULT 1, unit_price numeric(12,2) NOT NULL DEFAULT 0,
            amount numeric(12,2) NOT NULL DEFAULT 0
          );
          CREATE TABLE IF NOT EXISTS public.aios_payments (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), invoice_id uuid NOT NULL REFERENCES public.aios_invoices(id) ON DELETE CASCADE,
            amount numeric(12,2) NOT NULL, method varchar(80), reference varchar(255), status varchar(40) NOT NULL DEFAULT 'RECORDED',
            paid_at timestamptz, provider_response jsonb, created_by uuid, created_at timestamptz NOT NULL DEFAULT now()
          );
          ...
          CREATE TABLE IF NOT EXISTS public.aios_client_workspaces (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), client_name varchar(255) NOT NULL, client_email varchar(255), company_name varchar(255),
            owner_user_id uuid, status varchar(40) NOT NULL DEFAULT 'ONBOARDING', goals text, services jsonb NOT NULL DEFAULT '[]'::jsonb,
            created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
          );
          CREATE TABLE IF NOT EXISTS public.aios_client_onboarding (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), workspace_id uuid NOT NULL REFERENCES public.aios_client_workspaces(id) ON DELETE CASCADE,
            checklist jsonb NOT NULL DEFAULT '{}'::jsonb, completed_at timestamptz, updated_at timestamptz NOT NULL DEFAULT now(), UNIQUE(workspace_id)
          );
          CREATE TABLE IF NOT EXISTS public.aios_notifications (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid, type varchar(100) NOT NULL, title varchar(255) NOT NULL,
            body text, href varchar(1000), read boolean NOT NULL DEFAULT false, created_at timestamptz NOT NULL DEFAULT now()
          );
          ...
          CREATE TABLE IF NOT EXISTS public.aios_audit_logs (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid, action varchar(150) NOT NULL, target_type varchar(100), target_id varchar(255),
            before_state jsonb, after_state jsonb, provider_response jsonb, success boolean NOT NULL DEFAULT true, error text, created_at timestamptz NOT NULL DEFAULT now()
          );
          ...
          CREATE TABLE IF NOT EXISTS public.aios_agent_runs (
            id uuid PRIMARY KEY DEFAULT gen_random_uuid(), agent varchar(100) NOT NULL, status varchar(40) NOT NULL DEFAULT 'QUEUED',
            input jsonb, output jsonb, error text, user_id uuid, started_at timestamptz, completed_at timestamptz, created_at timestamptz NOT NULL DEFAULT now()
          );
          CREATE TABLE IF NOT EXISTS public.aios_forecast_snapshots (
            ...
            pipeline numeric(12,2) NOT NULL DEFAULT 0, weighted_pipeline numeric(12,2) NOT NULL DEFAULT 0, won_revenue numeric(12,2) NOT NULL DEFAULT 0,
            outstanding numeric(12,2) NOT NULL DEFAULT 0, methodology jsonb NOT NULL DEFAULT '{}'::jsonb, created_at timestamptz NOT NULL DEFAULT now()
          );
        `);
        initialized = true;
...
export async function getCommandCenter() {
  ...
  const [leadCounts, opportunities, proposals, approvals, campaigns, social, tasks, followUps, invoices, payments, clients, automations, notifications] = await Promise.all([
    ...
    prisma.$queryRawUnsafe<Array<{ status: string; count: bigint }>>(`SELECT status::text, count(*)::bigint AS count FROM public.social_contents GROUP BY status`).catch(() => []),
  ...
  const weighted = Object.entries(stageRows).reduce((sum, [s, x]) => sum + x.value * ({ OPEN: 0.2, QUALIFIED: 0.4, PROPOSAL: 0.65, WON: 1, LOST: 0 } as Record<string, number>)[s], 0);
  const invoiceRows = Object.fromEntries(invoices.map(x => [x.status, { count: toNumber(x.count), total: Number(x.total) }]));
  const revenue = Number(payments[0]?.total ?? 0) + (invoiceRows.PAID?.total ?? 0);
  return {
...
export async function writeAudit(input: { userId?: string | null; action: string; targetType?: string; targetId?: string; before?: unknown; after?: unknown; providerResponse?: unknown; success?: boolean; error?: string }) {
  const prisma = await db();
  await prisma.$executeRawUnsafe(
    `INSERT INTO public.aios_audit_logs (user_id, action, target_type, target_id, before_state, after_state, provider_response, success, error) VALUES ($1::uuid,$2,$3,$4,$5::jsonb,$6::jsonb,$7::jsonb,$8,$9)`,
    input.userId ?? null, input.action, input.targetType ?? null, input.targetId ?? null,
    JSON.stringify(input.before ?? null), JSON.stringify(input.after ?? null), JSON.stringify(input.providerResponse ?? null), input.success ?? true, input.error ?? null,
  );
}
...
export async function createApproval(input: { action: string; targetType: string; targetId?: string | null; payload: unknown; reason?: string | null; userId?: string | null }) {
  ...
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string }>>(
    `INSERT INTO public.aios_approvals (action,target_type,target_id,payload,reason,created_by) VALUES ($1,$2,$3,$4::jsonb,$5,$6::uuid) RETURNING id`,
    input.action, input.targetType, input.targetId ?? null, JSON.stringify(input.payload), input.reason ?? null, input.userId ?? null,
  );
...
export async function listAgents() {
  return [
    ['SALES', 'Find, qualify and advance prospects', ['leads','crm','sales-messages'], true],
    ['RESEARCH', 'Research websites, market signals and opportunities', ['research','opportunities'], true],
    ['CONTENT', 'Plan captions, scripts and content calendars', ['social','content'], true],
    ['CREATIVE', 'Turn briefs into original creative specifications', ['creative','social'], true],
    ['SEO', 'Audit and prioritize organic growth opportunities', ['seo','research'], true],
    ['ADS', 'Analyze configured ad data and recommend actions', ['ads','analytics'], true],
    ['OPERATIONS', 'Tasks, approvals, follow-ups and workflow health', ['tasks','automations'], true],
    ['CLIENT_SUCCESS', 'Onboarding, client health and reporting', ['clients','reports'], true],
