      ...
      <DashboardLayout>
        <main className="space-y-5">
          <section className="nexor-fade nexor-panel p-7">
            ...
            <div className="mt-5">
              <div className="font-mono text-[7px] tracking-[0.16em] text-[var(--text-muted)]">LEAD GENERATION</div>
              <h1 className="mt-1 text-3xl font-semibold tracking-[-0.04em]">Lead Finder</h1>
              <p className="mt-4 max-w-2xl text-[10px] leading-5 text-[var(--text-secondary)]">Targeted discovery → research → scoring → requirement matching → personalized drafts.</p>
            </div>
          </section>
          <LeadFinderWorkspace />
        </main>
      ...
      <DashboardLayout>
        <main className="space-y-5">
          <section className="nexor-fade nexor-panel p-7">
            <Link href="/dashboard" className="font-mono text-[7px] tracking-[0.16em] text-[var(--text-muted)]">← COMMAND CENTER</Link>
            <div className="mt-5">
              <div className="font-mono text-[7px] tracking-[0.16em] text-[var(--text-muted)]">SOCIAL MEDIA</div>
              <h1 className="mt-1 text-3xl font-semibold tracking-[-0.04em]">Content Calendar</h1>
            </div>
          </section>
      ...
      <DashboardLayout>
        <main className="space-y-5">
          <section className="nexor-fade nexor-panel p-7">
            <Link href="/dashboard" className="font-mono text-[7px] tracking-[0.16em] text-[var(--text-muted)]">← COMMAND CENTER</Link>
            <div className="mt-5">
              <div className="font-mono text-[7px] tracking-[0.16em] text-[var(--text-muted)]">{group.toUpperCase()}</div>
              <h1 className="mt-1 text-3xl font-semibold tracking-[-0.04em]">{title}</h1>
    ...
    <DashboardLayout>
      <main className="space-y-5">
        <section className="nexor-fade nexor-panel p-7">
          <Link href="/dashboard" className="font-mono text-[7px] tracking-[0.16em] text-[var(--text-muted)]">← COMMAND CENTER</Link>
          <div className="mt-5">
            <div className="font-mono text-[7px] tracking-[0.16em] text-[var(--text-muted)]">{tool.group.toUpperCase()}</div>
            <h1 className="mt-1 text-3xl font-semibold tracking-[-0.04em]">{tool.name}</h1>
            <p className="mt-4 max-w-2xl text-[10px] leading-5 text-[var(--text-secondary)]">{tool.description}</p>
          </div>
          <div className="mt-5 inline-flex rounded-full bg-[var(--surface-3)] px-3 py-1.5 font-mono text-[7px] tracking-[0.14em] text-[var(--text-muted)]">{tool.status === 'ready' ? 'LIVE MODULE' : 'INTEGRATION / BUILD'}</div>
        </section>
        <section className="nexor-panel p-6">
          <div className="text-[12px] font-semibold">{tool.status === 'ready' ? 'Ready' : 'Architecture ready'}</div>
          <div className="mt-2 text-[9px] text-[var(--text-muted)]">Provider credentials are separated from UI logic.</div>
        </section>
      </main>
    </DashboardLayout>
  );
