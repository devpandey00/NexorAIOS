...
export default function WhatsAppAutomationPage() {
  ...
  function toggleApproved(id: string) { setSelectedApproved((current) => { const next = new Set(current); next.has(id) ? next.delete(id) : next.add(id); return next; }); }
  function selectAllDrafts() { setSelectedDrafts(allDraftsSelected ? new Set() : new Set(drafts.map((item) => item.id))); }
  function selectAllApproved() { setSelectedApproved(allApprovedSelected ? new Set() : new Set(approved.map((item) => item.id))); }
  ...
  return (
    <main className="space-y-5">
      ...
      <section className="nexor-panel overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] p-5"><div><div className="text-[10px] font-semibold">3. Approved — ready to send</div><div className="mt-1 text-[8px] text-[var(--text-muted)]">Only approved + contactable records appear here.</div></div><div className="flex flex-wrap gap-2"><button type="button" disabled={!approved.length || loading} onClick={selectAllApproved} className="rounded-lg border border-[var(--border)] px-3 py-2 text-[8px] disabled:opacity-40">{allApprovedSelected ? 'CLEAR ALL' : 'SELECT ALL'}</button><button type="button" disabled={!selectedApproved.size || loading} onClick={() => void run('schedule', [...selectedApproved])} className="rounded-lg border border-[var(--border)] px-3 py-2 text-[8px] disabled:opacity-40">SCHEDULE</button><button type="button" disabled={!selectedApproved.size || loading} onClick={() => void run('send', [...selectedApproved])} className="rounded-lg bg-[var(--accent)] px-3 py-2 text-[8px] font-bold text-black disabled:opacity-40">SEND APPROVED NOW</button></div></div><div className="divide-y divide-[var(--border)]">{approved.map((item) => <article key={item.id} className="p-5"><div className="flex gap-3"><Check checked={selectedApproved.has(item.id)} onToggle={() => toggleApproved(item.id)} label={`Select ${item.lead.businessName}`} /><button type="button" disabled={loading} onClick={() => toggleApproved(item.id)} className="min-w-0 flex-1 cursor-pointer text-left disabled:cursor-not-allowed"><span className="flex flex-wrap items-center gap-2"><strong className="text-[10px]">{item.lead.businessName}</strong><span className="rounded bg-[var(--surface-2)] px-2 py-1 font-mono text-[7px]">{item.lead.whatsapp}</span><span className="text-[7px] text-emerald-400">APPROVED</span></span><span className="mt-3 block whitespace-pre-wrap text-[9px] leading-5 text-[var(--text-secondary)]">{item.message}</span></button></div></article>)}{!approved.length && <div className="p-10 text-center text-[9px] text-[var(--text-muted)]">No approved contactable messages.</div>}</div></section>
