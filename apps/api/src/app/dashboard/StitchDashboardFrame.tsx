...
export default function StitchDashboardFrame() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (profile?.success && profile.profile) {
        const serverProfile = profile.profile as Record<string, unknown>;
        setSettings((current) => ({ ...current, ...serverProfile, avatar: typeof serverProfile.avatar_url === 'string' ? serverProfile.avatar_url : typeof serverProfile.avatar === 'string' ? serverProfile.avatar : current.avatar }));
      }
  ...
  async function saveSettings(next: SettingsState) {
    setSettings(next);
    try {
      const res = await fetch('/api/founder/profile', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ profile: { ...next, avatar_url: next.avatar } }) });
      const body = await res.json();
