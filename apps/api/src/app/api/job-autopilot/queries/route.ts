...
function buildQueries() {
  return locations.flatMap((location) => roles.flatMap((role) => [
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ success: true, queries: buildQueries(), roles, locations, count: buildQueries().length });
}
...
export async function POST(req: NextRequest) {
  ...
  const queries = requestedLocations.flatMap((location) => requestedRoles.flatMap((role) => [
    `"${role}" "${location}" jobs hiring`,
    `"${role}" "${location}" apply`,
    `site:linkedin.com/jobs "${role}" "${location}"`,
    `site:indeed.com "${role}" "${location}"`,
