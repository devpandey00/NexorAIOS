...
async function requestWithRetry(url: string, init: RequestInit) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) { const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS); try { return await fetch(url, { ...init, signal: controller.signal, cache: 'no-store' }); } catch (error) { lastError = error; if (attempt < MAX_ATTEMPTS) await new Promise((resolve) => setTimeout(resolve, 300 * attempt)); } finally { clearTimeout(timeout); } }
  throw lastError instanceof Error ? lastError : new Error(String(lastError));
}
async function sendWhatsApp(to: string, message: string) {
  const token = process.env.WHATSAPP_ACCESS_TOKEN; const phoneNumberId = process.env.WHATSAPP_PHONE_NUMBER_ID; const version = process.env.WHATSAPP_API_VERSION ?? 'v23.0'; if (!token || !phoneNumberId) throw new Error('WhatsApp credentials are not configured');
  const response = await requestWithRetry(`https://graph.facebook.com/${version}/${phoneNumberId}/messages`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ messaging_product: 'whatsapp', recipient_type: 'individual', to: to.replace(/\D/g,''), type: 'text', text: { preview_url: false, body: message } }) });
  const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data?.error?.message ?? `WhatsApp send failed (${response.status})`); return data?.messages?.[0]?.id as string | undefined;
}
async function sendEmail(to: string, message: string) {
  const apiKey = process.env.RESEND_API_KEY; const from = process.env.OUTREACH_FROM_EMAIL; if (!apiKey || !from) throw new Error('Email credentials are not configured');
  ...
  const response = await requestWithRetry('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [to], subject, text }) });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let providerMessageId: string | undefined; let conversationChannel: ConversationChannel; let recipient: string;
    if (outreach.channel === OutreachChannel.WHATSAPP) { recipient = outreach.lead.whatsapp ?? ''; conversationChannel = ConversationChannel.WHATSAPP; if (!recipient) throw new Error('Lead has no WhatsApp number'); providerMessageId = await sendWhatsApp(recipient, outreach.message); }
    else if (outreach.channel === OutreachChannel.EMAIL) { recipient = outreach.lead.email ?? ''; conversationChannel = ConversationChannel.EMAIL; if (!recipient) throw new Error('Lead has no email address'); providerMessageId = await sendEmail(recipient, outreach.message); }
    else throw new Error(`Sending for ${outreach.channel} is not configured yet`);
    const now = new Date();
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (claimed.count !== 1) { const current = await tx.outreach.findUnique({ where: { id } }); if (current?.status === OutreachStatus.SENT) return current; throw new Error('Outreach was claimed by another sender'); }
