...
function normalizeChannel(value: unknown): OutreachChannel | null {
  if (Object.values(OutreachChannel).includes(value as OutreachChannel))
...
function buildMessage(
  ...
  lead: { businessName: string; ownerName: string | null },
  context: string,
) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!leadId || !channel || !supported.includes(channel))
      return NextResponse.json(
        { success: false, error: 'leadId and a supported outreach channel are required' },
        { status: 400 },
      );
      ...
      const created = await tx.outreach.create({
        data: {
          ...
          message: buildMessage(
            ...
            lead,
            context ||
              'Your current digital presence looks like it has room for measurable improvements.',
          ),
        },
      });
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (!existing)
      return NextResponse.json({ success: false, error: 'Outreach not found' }, { status: 404 });
    const awaitingApproval: OutreachStatus[] = [
      OutreachStatus.DRAFT,
      OutreachStatus.APPROVAL_REQUIRED,
    ];
    if (!awaitingApproval.includes(existing.status))
      return NextResponse.json(
        {
          success: false,
          error: `Outreach is not awaiting approval. Current status: ${existing.status}`,
        },
        { status: 409 },
      );
