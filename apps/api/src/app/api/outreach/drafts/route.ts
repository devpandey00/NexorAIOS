...
function parseNotes(notes: string | null) {
  ...
  try { const parsed = JSON.parse(notes); return parsed && typeof parsed === 'object' ? parsed as Record<string, any> : {} as Record<string, any>; }
...
function validateBusinessLead(lead: { businessName: string; website: string | null; notes: string | null; whatsapp: string | null }) {
  ...
  if (lead.website) { try { if (NON_BUSINESS_PATHS.test(new URL(lead.website).pathname)) return { ok: false, reason: 'Blocked job/content/listing website' }; } catch { return { ok: false, reason: 'Invalid lead website' }; }
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const lead = await prisma.lead.findUnique({ where: { id: leadId } }); if (!lead) return NextResponse.json({ success: false, error: 'Lead not found' }, { status: 404 });
    const validation = validateBusinessLead(lead); if (!validation.ok) return NextResponse.json({ success: false, error: validation.reason }, { status: 422 });
    ...
    if (channel === OutreachChannel.WHATSAPP) {
      const parsed = validation.parsed ?? {}; const generated = await outreachService.generate({ businessName: lead.businessName, ownerName: lead.ownerName, niche: lead.niche, country: lead.country, website: lead.website, whatsapp: lead.whatsapp, auditScore: lead.auditScore, notes: lead.notes, verifiedResearch: parsed.research ?? {}, verifiedScore: parsed.score ?? {}, leadMetadata: { leadType: validation.leadType, source: validation.source }, uniquenessInstruction: `Create a unique first-contact WhatsApp message for ${lead.businessName}. Use one or two specific verified findings from this lead only. ${context}` });
      message = typeof generated?.whatsapp === 'string' ? generated.whatsapp.trim() : ''; if (!message) return NextResponse.json({ success: false, error: 'AI could not produce a personalized WhatsApp draft' }, { status: 502 });
    } else message = buildMessage(channel, lead, context || 'Your current digital presence looks like it has room for measurable improvements.');
    const outreach = await prisma.$transaction(async (tx) => { const created = await tx.outreach.create({ data: { leadId, channel, status: OutreachStatus.APPROVAL_REQUIRED, message } }); await tx.lead.update({ where: { id: leadId }, data: { status: LeadStatus.PITCH_READY } }); return created; });
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    const existing = await prisma.outreach.findUnique({ where: { id }, include: { lead: true } }); if (!existing) return NextResponse.json({ success: false, error: 'Outreach not found' }, { status: 404 });
    const awaitingApproval: OutreachStatus[] = [OutreachStatus.DRAFT, OutreachStatus.APPROVAL_REQUIRED]; if (!awaitingApproval.includes(existing.status)) return NextResponse.json({ success: false, error: `Outreach is not awaiting approval. Current status: ${existing.status}` }, { status: 409 });
    if (action === 'approve') { const validation = validateBusinessLead(existing.lead); if (!validation.ok) return NextResponse.json({ success: false, error: validation.reason }, { status: 422 }); if (existing.channel === OutreachChannel.WHATSAPP && !existing.lead.whatsapp) return NextResponse.json({ success: false, error: 'NOT CONTACTABLE: WhatsApp number missing' }, { status: 422 }); }
    const status = action === 'approve' ? OutreachStatus.APPROVED : OutreachStatus.CANCELLED; const scheduledAt = action === 'approve' ? new Date(Date.now() + 5 * 60 * 1000) : null;
