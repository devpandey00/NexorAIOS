...
export async function POST(req: NextRequest) {
  try {
    ...
    if (![OutreachStatus.DRAFT, OutreachStatus.APPROVAL_REQUIRED].includes(current.status)) return NextResponse.json({ success: false, error: 'Outreach is not pending approval' }, { status: 409 });
