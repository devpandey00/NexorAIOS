...
function buildFollowUpMessage(lead: { businessName: string; notes: string | null }) {
  let requirement = 'improving your online lead generation'; let finding = '';
...
export async function GET(req: Request) {
  ...
  try {
    ...
    for (const candidate of due) {
      const claimed = await prisma.followUp.updateMany({ where: { id: candidate.id, status: FollowUpStatus.PENDING }, data: { status: FollowUpStatus.SCHEDULED, notes: `${candidate.notes ?? ''}\nClaimed by follow-up worker at ${new Date().toISOString()}`.trim() } });
      if (claimed.count !== 1) continue;
      try {
        if (candidate.attemptCount >= candidate.maxAttempts) { await prisma.followUp.update({ where: { id: candidate.id }, data: { status: FollowUpStatus.CANCELLED } }); continue; }
        const terminal = ['REPLIED','MEETING_BOOKED','PROPOSAL_SENT','WON','LOST'].includes(candidate.lead.status) || Boolean(await prisma.conversation.findFirst({ where: { leadId: candidate.leadId, status: { in: ['NOT_INTERESTED','WRONG_PERSON','UNSUBSCRIBE'] } }, select: { id: true } }));
        if (terminal) { await prisma.followUp.update({ where: { id: candidate.id }, data: { status: FollowUpStatus.CANCELLED, notes: 'Cancelled because reply intelligence marked the lead as not contactable for follow-up.' } }); skipped++; continue; }
        const channel = candidate.lead.whatsapp ? OutreachChannel.WHATSAPP : candidate.lead.email ? OutreachChannel.EMAIL : null;
        if (!channel) { await prisma.followUp.update({ where: { id: candidate.id }, data: { status: FollowUpStatus.CANCELLED, notes: 'No supported contact channel' } }); skipped++; continue; }
        ...
        if (existing) { await prisma.followUp.update({ where: { id: candidate.id }, data: { status: FollowUpStatus.COMPLETED, attemptCount: { increment: 1 }, notes: `${candidate.notes ?? ''}\nSkipped because active outreach already exists.`.trim() } }); skipped++; continue; }
        const scheduledAt = new Date(Date.now() + 5 * 60 * 1000);
        await prisma.$transaction(async (tx) => { await tx.outreach.create({ data: { leadId: candidate.leadId, channel, status: OutreachStatus.APPROVED, message: buildFollowUpMessage(candidate.lead), approvedAt: new Date(), scheduledAt } }); await tx.followUp.update({ where: { id: candidate.id }, data: { status: FollowUpStatus.COMPLETED, attemptCount: { increment: 1 }, notes: `${candidate.notes ?? ''}\nAutomatic follow-up queued for ${scheduledAt.toISOString()}`.trim() } }); });
        queued++;
      } catch (error) { await prisma.followUp.update({ where: { id: candidate.id }, data: { status: FollowUpStatus.PENDING, attemptCount: { increment: 1 }, notes: `${candidate.notes ?? ''}\nWorker error: ${error instanceof Error ? error.message : String(error)}`.trim() } }).catch(() => undefined); }
    }
    return NextResponse.json({ success: true, due: due.length, queued, skipped });
  } catch (error) { console.error('[CRON FOLLOWUP ERROR]', error); return NextResponse.json({ success: false, error: error instanceof Error ? error.message : String(error) }, { status: 500 }); }
}
