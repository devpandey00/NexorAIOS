...
export async function GET(req: NextRequest) {
  ...
  try {
    const claimed = await claimScheduledSocialContent(20); const results: Array<{ id: string; success: boolean; error?: string }> = [];
    for (const post of claimed) { try { await publishSocialPost(post.id); results.push({ id: post.id, success: true }); } catch (error) { const message = error instanceof Error ? error.message : String(error); await updateSocialContent(post.id, { status: 'FAILED', error: message }).catch(() => undefined); results.push({ id: post.id, success: false, error: message }); } }
