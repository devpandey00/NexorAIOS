...
function fallbackContent(input: {
  ...
  return {
    title,
    caption,
    hashtags: ['#DigitalMarketing', '#LeadGeneration', nicheTag ? `#${nicheTag}` : '#NexorMedia'],
  };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      const raw = await geminiAnalyze(`
Create one high-quality social media post for Nexor Media.
Return ONLY valid JSON with keys: title, caption, hashtags.
Platform: ${platform}
Niche: ${niche}
Goal: ${goal}
Offer: ${offer || 'digital marketing services'}
Audience: ${audience || 'business owners'}
Tone: ${tone}
Rules: no fake claims, no invented statistics, natural human copy, concise, useful, clear CTA, platform appropriate.
