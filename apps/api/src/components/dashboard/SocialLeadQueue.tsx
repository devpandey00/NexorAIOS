...
export default function SocialLeadQueue() {
  ...
  async function approve(id: string) {
    ...
    const data = await response.json();
    setMessage(response.ok && data.success ? 'Draft approved. Open the prospect profile and send it manually.' : data.error ?? 'Approval failed');
    await load();
  }
  ...
  return (
    <section className="space-y-5">
      <div className="nexor-panel p-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="font-mono text-[7px] tracking-[0.15em] text-[var(--accent)]">SOCIAL PROSPECT DATABASE</div><h2 className="mt-2 text-xl font-semibold">Find businesses with real social signals.</h2><p className="mt-1 max-w-2xl text-[9px] leading-5 text-[var(--text-secondary)]">Discovery feeds this queue. Public social links are retained for research; AI creates personalized drafts, but cold social DMs remain manual.</p></div><button type="button" onClick={() => void load()} className="rounded-lg border border-[var(--border)] px-3 py-2 text-[8px]">{loading ? 'LOADING…' : 'REFRESH'}</button></div>
