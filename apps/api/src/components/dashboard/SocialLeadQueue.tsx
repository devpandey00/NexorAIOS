...
export default function SocialLeadQueue() {
  ...
  async function load() {
    ...
    try {
      ...
      setDrafts((draftData.drafts ?? []).filter((item: Draft) => ['INSTAGRAM', 'FACEBOOK', 'LINKEDIN'].includes(item.channel)));
  ...
  return (
    <section className="space-y-5">
      <div className="nexor-panel p-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="font-mono text-[7px] tracking-[0.15em] text-[var(--accent)]">SOCIAL PROSPECT DATABASE</div><h2 className="mt-2 text-xl font-semibold">Find businesses with real social signals.</h2><p className="mt-1 max-w-2xl text-[9px] leading-5 text-[var(--text-secondary)]">Discovery feeds this queue. Public social links are retained for research; AI creates personalized drafts, while cold social DMs remain manual.</p></div><div className="flex gap-2"><button type="button" onClick={() => void createBulkDrafts()} disabled={bulkLoading} className="rounded-lg bg-[var(--accent)] px-3 py-2 text-[8px] font-bold text-black disabled:opacity-50">{bulkLoading ? 'BUILDING…' : 'DRAFT ALL QUALIFIED'}</button><button type="button" onClick={() => void load()} className="rounded-lg border border-[var(--border)] px-3 py-2 text-[8px]">{loading ? 'LOADING…' : 'REFRESH'}</button></div></div>
      ...
      <section className="nexor-panel overflow-hidden"><div className="border-b border-[var(--border)] p-5"><div className="text-[11px] font-semibold">Social message approval queue</div><div className="mt-1 text-[8px] text-[var(--text-muted)]">Approve a message, open the public profile, then send it manually. Email and WhatsApp drafts can be approved and sent through Nexor's existing sender.</div></div><div className="divide-y divide-[var(--border)]">{drafts.map((draft) => <article key={draft.id} className="p-5"><div className="flex flex-col gap-4 lg:flex-row lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><strong className="text-[9px]">{draft.lead.businessName}</strong><span className="rounded bg-[var(--surface-2)] px-2 py-1 font-mono text-[7px]">{draft.channel}</span><span className="text-[7px] text-[var(--text-muted)]">{draft.status}</span></div><p className="mt-3 max-w-3xl whitespace-pre-wrap text-[9px] leading-5 text-[var(--text-secondary)]">{draft.message}</p></div><div className="flex shrink-0 gap-2">{draft.status !== 'APPROVED' && <button type="button" onClick={() => void approve(draft.id)} className="rounded-lg bg-[var(--accent)] px-3 py-2 text-[7px] font-bold text-black">APPROVE</button>}{(draft.lead.socialProfiles ?? []).filter((profile) => profile.platform === draft.channel).map((profile) => <a key={profile.url} href={profile.url} target="_blank" rel="noreferrer" className="rounded-lg border border-[var(--border)] px-3 py-2 text-[7px]">OPEN PROFILE ↗</a>)}</div></div></article>)}{!drafts.length && <div className="p-10 text-center text-[9px] text-[var(--text-muted)]">No social drafts waiting for approval.</div>}</div></section>
    </section>
  );
}
