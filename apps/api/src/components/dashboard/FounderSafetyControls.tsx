...
export default function FounderSafetyControls({ controls }: { controls: Control[] }) {
  ...
  return (
    <section className="nexor-panel mb-5 overflow-hidden">
      ...
      <div className="grid gap-3 p-5 md:grid-cols-2">
        <ControlCard title="MASTER AUTOPILOT" description="Controls the autonomous operating loop." enabled={master} busy={busy === 'master_autopilot'} onToggle={() => void toggle('master_autopilot', !master)} />
        <ControlCard title="OUTBOUND COMMUNICATIONS" description="Emergency gate for email, WhatsApp and social publishing workers." enabled={outbound} busy={busy === 'outbound_enabled'} danger={!outbound} onToggle={() => void toggle('outbound_enabled', !outbound)} />
      </div>
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] p-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <button type="button" disabled={busy} onClick={onToggle} className={['relative h-7 w-12 shrink-0 rounded-full border transition disabled:opacity-50', enabled ? 'border-emerald-500/30 bg-emerald-500/15' : danger ? 'border-red-500/30 bg-red-500/10' : 'border-[var(--border)] bg-[var(--surface)]'].join(' ')} aria-label={`${enabled ? 'Disable' : 'Enable'} ${title}`}>
