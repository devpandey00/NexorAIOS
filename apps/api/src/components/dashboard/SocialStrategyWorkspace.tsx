...
type Strategy = { opportunity: string; pillar: string; audience: string; objective: string; format: string; angle: string; hook: string; keyMessage: string; cta: string; platform: string; postingWindow: string; creativeDirection: string; ideas: Idea[] };
...
export default function SocialStrategyWorkspace() {
  ...
  async function generate() {
    ...
    try {
      const response = await fetch('/api/social/strategy', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ platform, niche, goal, audience, offer, trendTopic, trendOpportunity }) });
      const data = await response.json();
  ...
  return <div className="space-y-5">
    <section className="nexor-panel p-6">
      <div className="font-mono text-[7px] tracking-[0.15em] text-[var(--accent)]">CONTENT STRATEGY ENGINE</div>
      <h2 className="mt-2 text-lg font-semibold text-[var(--text)]">Trend → opportunity → strategy → ideas</h2>
