  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      const response = await fetch(url, {
        headers: { 'user-agent': 'Mozilla/5.0 NexorAIOS/1.0', accept: 'text/html,application/xhtml+xml' },
...
async function queryEngine(query: string, engine: 'ddg' | 'bing' | 'searx'): Promise<SearchResult[]> {
  ...
  if (engine === 'ddg') url.searchParams.set('kl', 'us-en');
...
export async function freeWebSearch(query: string): Promise<{ leads: Lead[]; provider: string; errors: string[] }> {
  ...
  const enriched = await Promise.all(candidates.map(async (result) => ({ result, name: await resolveName(result) })));
  return {
    leads: enriched.map(({ result, name }) => ({ name, website: result.url })),
    provider: process.env.SEARXNG_URL ? 'free-auto:ddg>bing>searxng' : 'free-auto:ddg>bing',
    errors,
  };
}
