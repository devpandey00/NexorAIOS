...
function locationCandidates(query: string): string[] {
  ...
  for (const city of Object.keys(KNOWN_LOCATIONS)) if (new RegExp(`\\b${city.replace(' ', '\\s+')}\\b`, 'i').test(query)) candidates.push(city);
...
async function geocodeLocation(query: string): Promise<{ lat: number; lon: number } | null> {
  for (const location of locationCandidates(query)) {
    const known = KNOWN_LOCATIONS[location.toLowerCase()];
...
async function overpassSearch(query: string): Promise<Lead[]> {
  ...
  const radius = /\b(?:dubai|abu dhabi|sharjah|london|new york|toronto|sydney|melbourne|singapore)\b/i.test(query) ? 25000 : 15000;
  // One focused category is substantially faster and more reliable on public Overpass instances.
  const filter = filters[0] ?? '["office"]';
  const body = `[out:json][timeout:7];(nwr${filter}(around:${radius},${point.lat},${point.lon}););out center tags;`;
...
export async function openStreetMapSearch(query: string): Promise<Lead[]> {
  ...
  // For known local-business categories, go directly to Overpass first. Nominatim is a
  // geocoder/search index and is not a reliable category search for phrases like "dentists Dubai".
  if (categoryFilters(normalized)[0] !== '["office"]') {
    const direct = await overpassSearch(normalized);
    if (direct.length) return direct;
  }

  for (const variant of queryVariants(normalized)) {
    const leads = await nominatimSearch(variant);
