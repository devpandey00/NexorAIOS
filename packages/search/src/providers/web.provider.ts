...
async function resolveBusinessName(result: SearchResult): Promise<string> {
  ...
  if (html) {
    ...
    for (const pattern of candidates) {
      const match = html.match(pattern); const value = match?.[1] ? stripHtml(decodeHtml(match[1])).replace(/\s*[|·–—-]\s*(home|homepage|official website|digital marketing.*)$/i,'').trim() : '';
...
export async function webSearch(query: string): Promise<Lead[]> {
  ...
  const searchQueries = [normalizedQuery, `"${normalizedQuery}" business`, `${normalizedQuery} official website`]; const allResults: SearchResult[] = [];
  for (const searchQuery of searchQueries) { const url = new URL(SEARCH_URL); url.searchParams.set('q',searchQuery); url.searchParams.set('kl','us-en'); const html = await fetchWithRetry(url.toString()); if (html) allResults.push(...extractResults(html)); await new Promise((resolve) => setTimeout(resolve,700)); }
  const seenDomains = new Set<string>(); const leads: Lead[] = [];
  for (const result of allResults) { const domain = normalizeDomain(result.url); if (!domain || seenDomains.has(domain) || !isUsefulBusinessWebsite(result)) continue; seenDomains.add(domain); leads.push({ name: await resolveBusinessName(result), website: result.url }); if (leads.length >= 25) break; }
  return leads;
}
