...
const NON_BUSINESS_TITLE_PATTERNS = [/\bbest\b/i,/\btop\b/i,/\blist\b/i,/\bdirectory\b/i,/\bguide\b/i,/\broundup\b/i,/\barticles?\b/i,/\bcompanies\b/i,/\bdesigners\b.*\bto know\b/i,/\byou need to know\b/i];
...
export async function webSearch(query: string): Promise<Lead[]> {
  ...
  for (const result of allResults) {
    ...
    seenDomains.add(domain); leads.push({ name: result.title, website: result.url });
