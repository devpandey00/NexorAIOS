  ...
  async search(query: string): Promise<LeadSearchResult> {
    ...
    if (mode === 'auto' || mode === 'free') {
      try {
        ...
        if (result.leads.length > 0) return { success: true, count: result.leads.length, leads: result.leads, provider: result.provider, providerErrors: [...errors, ...result.errors] };
      ...
      try {
        ...
        if (leads.length > 0) return { success: true, count: leads.length, leads, provider: 'openstreetmap', providerErrors: errors };
