...
const workflows = ['lead_generation','sales_machine','lead_to_outreach','social_content','opportunity_discovery','crm','research','website_audit','whatsapp','email','proposal'] as const;
const routeSchema = z.object({ workflow: z.enum(workflows), confidence: z.number().min(0).max(1), reason: z.string().min(1), input: z.record(z.string(), z.unknown()).default({}) });
export type CommandRoute = z.infer<typeof routeSchema>;

const FALLBACKS: Array<[RegExp, CommandRoute['workflow']]> = [
  [/score|qualif|sales machine|find .* and .*research|find .*prospect.*research/i, 'sales_machine'],
...
function fallback(command: string): CommandRoute {
  ...
  return { workflow: match?.[1] ?? 'research', confidence: match ? 0.72 : 0.35, reason: match ? 'Matched the command to a supported Nexor workflow.' : 'No exact intent match; routed to research for safe inspection.', input: { command } };
}
  ...
  async route(command: string, context: Record<string, unknown> = {}): Promise<CommandRoute> {
    ...
    try {
      return await analyzerService.analyze<CommandRoute>({
        prompt: `You are NexorAIOS command router. Choose exactly one workflow from: ${workflows.join(', ')}. Use sales_machine when the user wants a prospecting pipeline combining discovery, research, qualification/scoring and CRM. Convert the user's command into structured input. Do not claim an action happened; only route it. Return JSON only. User command: ${text}\nContext: ${JSON.stringify(context)}`,
        schema: routeSchema,
      });
