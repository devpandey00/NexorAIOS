...
export class LeadRepository extends BaseRepository {
  async create(data: Prisma.LeadCreateInput): Promise<Lead> { return this.writeClient.lead.create({ data }); }
  async findById(id: string): Promise<Lead | null> { return this.readClient.lead.findUnique({ where: { id } }); }
  ...
  async findMany(filters: LeadFilters = {}): Promise<PaginatedResult<Lead>> {
    ...
    const [data, total] = await this.readClient.$transaction([this.readClient.lead.findMany({ where, skip, take, orderBy: { createdAt: 'desc' } }), this.readClient.lead.count({ where })]);
    return this.buildPaginatedResult(data, total, page, pageSize);
  }
  async update(id: string, data: Prisma.LeadUpdateInput): Promise<Lead> { return this.writeClient.lead.update({ where: { id }, data }); }
  async delete(id: string): Promise<Lead> { return this.writeClient.lead.delete({ where: { id } }); }
  async changeStatus(id: string, status: LeadStatus): Promise<Lead> { return this.writeClient.lead.update({ where: { id }, data: { status } }); }
  async count(): Promise<number> { return this.readClient.lead.count(); }
