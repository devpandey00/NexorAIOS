-- NexorAIOS AIOS platform tables.
-- Idempotent, additive migration replacing runtime schema creation in the API.
-- No existing data is dropped or rewritten.
...
CREATE TABLE IF NOT EXISTS public.aios_contacts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid REFERENCES public.aios_companies(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.aios_automation_runs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  automation_id uuid NOT NULL REFERENCES public.aios_automations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.aios_invoice_items (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  invoice_id uuid NOT NULL REFERENCES public.aios_invoices(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.aios_client_onboarding (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id uuid NOT NULL REFERENCES public.aios_client_workspaces(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.aios_forecast_snapshots (
  ...
  pipeline numeric(12,2) NOT NULL DEFAULT 0,
