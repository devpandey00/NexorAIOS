...
-- intentionally idempotent and restores auth + CRM/automation tables without
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname='set_users_updated_at' AND pronamespace='public'::regnamespace) THEN
    CREATE FUNCTION public.set_users_updated_at() RETURNS TRIGGER LANGUAGE plpgsql AS $fn$
    BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END;
    $fn$;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='users_updated_at' AND tgrelid='public.users'::regclass) THEN
...
DECLARE item text;
BEGIN
  FOREACH item IN ARRAY ARRAY['campaign_status','job_status','job_type','outreach_status','outreach_channel','conversation_channel','message_direction','follow_up_status','task_status','social_platform'] LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname=item AND typnamespace='public'::regnamespace) THEN
...
CREATE TABLE IF NOT EXISTS public.campaign_leads (
  ...
  created_at timestamptz(6) NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT campaign_leads_pkey PRIMARY KEY (id)
);
CREATE TABLE IF NOT EXISTS public.jobs (
  id uuid NOT NULL DEFAULT gen_random_uuid(), campaign_id uuid, lead_id uuid, type public.job_type NOT NULL,
  status public.job_status NOT NULL DEFAULT 'QUEUED', attempts integer NOT NULL DEFAULT 0, "maxAttempts" integer NOT NULL DEFAULT 3,
  payload jsonb, result jsonb, error text, started_at timestamptz(6), completed_at timestamptz(6),
  created_at timestamptz(6) NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamptz(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS public.outreach (
  id uuid NOT NULL DEFAULT gen_random_uuid(), lead_id uuid NOT NULL, campaign_id uuid, channel public.outreach_channel NOT NULL,
  status public.outreach_status NOT NULL DEFAULT 'DRAFT', message text NOT NULL, approved_at timestamptz(6), scheduled_at timestamptz(6),
  sent_at timestamptz(6), provider_message_id varchar(255), error text, created_at timestamptz(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS public.conversations (
  ...
  status varchar(50) NOT NULL DEFAULT 'OPEN', last_message_at timestamptz(6), created_at timestamptz(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamptz(6) NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT conversations_pkey PRIMARY KEY (id)
);
...
CREATE TABLE IF NOT EXISTS public.activity_events (
  ...
  metadata jsonb, created_at timestamptz(6) NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT activity_events_pkey PRIMARY KEY (id)
);
